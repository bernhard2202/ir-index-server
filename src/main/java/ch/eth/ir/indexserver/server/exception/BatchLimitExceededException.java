 ...
 * a single batch request than {@link ServerProperties#MAX_BATCH_REQ_ALLOWED} 
 * allows
 */
public class BatchLimitExceededException extends WebApplicationException {
	public BatchLimitExceededException() {
		super(Response.status(Response.Status.BAD_REQUEST).entity(String
				.format("Maximum batch size is %d within a single request!", ServerProperties.MAX_BATCH_REQ_ALLOWED))
